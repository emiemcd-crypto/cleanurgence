...
export default function Inscription() {
  ...
  const handleSubmit = () => {
    ...
    if (role === 'Prestataire' && !siret) {
      alert('Veuillez entrer votre numéro SIRET')
      return
    }
    alert(`Compte ${role} créé pour ${nom} ! ✅`)
  }
          ...
          <>
            ...
            <div style={{marginBottom:'14px'}}>
              <label style={{fontSize:'0.75rem',fontWeight:'700',color:'#555',textTransform:'uppercase',letterSpacing:'0.06em',display:'block',marginBottom:'6px'}}>
                {role==='Entreprise'?'Nom de la société':'Prénom et nom'}
              </label>
              <input value={nom} onChange={e=>setNom(e.target.value)} placeholder={role==='Entreprise'?'Ex: Conciergerie Azur':'Ex: Sophie Bertrand'} style={{width:'100%',padding:'12px 14px',borderRadius:'10px',border:'1.5px solid #eee',fontFamily:'sans-serif',fontSize:'0.9rem',outline:'none',boxSizing:'border-box'}} onFocus={e=>e.target.style.borderColor='#FF4D00'} onBlur={e=>e.target.style.borderColor='#eee'}/>
            </div>
        ...
        {step === 3 && (
          <>
            ...
            <div style={{background:'rgba(255,77,0,0.05)',border:'1px solid rgba(255,77,0,0.15)',borderRadius:'10px',padding:'12px 14px',marginBottom:'20px',fontSize:'0.82rem',color:'#FF4D00'}}>
              ℹ️ Ces documents sont obligatoires pour garantir la confiance sur la plateforme
            </div>
            ...
            <div style={{marginBottom:'16px'}}>
              ...
              <input value={siret} onChange={e=>setSiret(e.target.value)} placeholder="Ex: 123 456 789 00012" maxLength={17} style={{width:'100%',padding:'12px 14px',borderRadius:'10px',border:'1.5px solid #eee',fontFamily:'sans-serif',fontSize:'0.9rem',outline:'none',boxSizing:'border-box',letterSpacing:'0.05em'}} onFocus={e=>e.target.style.borderColor='#FF4D00'} onBlur={e=>e.target.style.borderColor='#eee'}/>
              <div style={{fontSize:'0.72rem',color:'#888',marginTop:'4px'}}>14 chiffres — trouvez-le sur societe.com</div>
            </div>
            ...
            <div style={{marginBottom:'16px'}}>
              ...
              <input id="upload-attestation" type="file" accept=".pdf" style={{display:'none'}} onChange={e=>e.target.files[0] && alert(`✅ Fichier "${e.target.files[0].name}" sélectionné`)}/>
            </div>

            <div style={{marginBottom:'20px'}}>
              <label style={{fontSize:'0.75rem',fontWeight:'700',color:'#555',textTransform:'uppercase',letterSpacing:'0.06em',display:'block',marginBottom:'6px'}}>Assurance RC Pro (optionnel)</label>
              <div style={{border:'2px dashed #ddd',borderRadius:'10px',padding:'20px',textAlign:'center',cursor:'pointer',background:'#FAFAF7'}} onClick={()=>document.getElementById('upload-assurance').click()}>
                <div style={{fontSize:'1.5rem',marginBottom:'6px'}}>🛡️</div>
                <div style={{fontSize:'0.85rem',fontWeight:'600',color:'#555'}}>Cliquez pour uploader</div>
                <div style={{fontSize:'0.75rem',color:'#888',marginTop:'3px'}}>PDF · Max 5 Mo</div>
              </div>
              <input id="upload-assurance" type="file" accept=".pdf" style={{display:'none'}} onChange={e=>e.target.files[0] && alert(`✅ Fichier "${e.target.files[0].name}" sélectionné`)}/>
            </div>
          ...
          <>
            <div style={{textAlign:'center',padding:'20px 0'}}>
              ...
              <div style={{color:'#888',fontSize:'0.88rem',lineHeight:'1.6',marginBottom:'24px'}}>
                Votre compte <strong>{role}</strong> pour <strong>{nom}</strong> est prêt à être créé.
              </div>
              <div style={{background:'#FAFAF7',borderRadius:'12px',padding:'16px',textAlign:'left',marginBottom:'24px'}}>
                ...
                <div style={{fontSize:'0.78rem',color:'#0CB86A',fontWeight:'600'}}>✅ Documents vérifiés</div>
              </div>
              <button onClick={handleSubmit} style={{width:'100%',padding:'14px',background:'#FF4D00',color:'white',border:'none',borderRadius:'12px',fontFamily:'sans-serif',fontWeight:'700',fontSize:'1rem',cursor:'pointer',marginBottom:'12px'}}>
                ✅ Créer mon compte
              </button>

...
export default function Home() {
  ...
  if (!role) return (
    <div style={{minHeight:'100vh',background:'#FAFAF7',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',fontFamily:'sans-serif',padding:'20px'}}>
      ...
      <p style={{color:'#888',marginBottom:'48px',fontSize:'0.95rem'}}>Trouvez un prestataire disponible · Immédiatement</p>
      <div style={{display:'flex',gap:'20px',flexWrap:'wrap',justifyContent:'center'}}>
        <div onClick={()=>setRole('conciergerie')} style={{background:'white',border:'2px solid #eee',borderRadius:'16px',padding:'32px 28px',cursor:'pointer',textAlign:'center',width:'220px',transition:'all 0.2s'}} onMouseOver={e=>e.currentTarget.style.borderColor='#FF4D00'} onMouseOut={e=>e.currentTarget.style.borderColor='#eee'}>
    ...
    <div style={{minHeight:'100vh',background:'#FAFAF7',fontFamily:'sans-serif',padding:'32px 24px'}}>
      <div style={{maxWidth:'800px',margin:'0 auto'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'32px'}}>
          ...
          <button onClick={()=>setRole(null)} style={{background:'none',border:'1px solid #ddd',borderRadius:'8px',padding:'8px 16px',cursor:'pointer',fontSize:'0.85rem'}}>← Changer de rôle</button>
        </div>
        {role === 'conciergerie' ? (
          <div>
            ...
            <button style={{background:'#FF4D00',color:'white',border:'none',borderRadius:'10px',padding:'12px 24px',fontWeight:'700',fontSize:'1rem',cursor:'pointer',marginBottom:'32px'}}>+ Publier une urgence</button>
